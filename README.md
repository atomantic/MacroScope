...
The comparison response contains the immutable assumptions, population aggregates, all three strategy outcomes, the ten-year projection and verdict, the inflation stress test, accounting residuals, decile results, sector demand, and caveats. The uncertainty response adds declared assumption metadata, direction-checked rank-factor dependencies with observed Spearman correlations, p10/p50/p90 outcome and trajectory bands, verdict frequencies, influences, and interactions. Borrowing and asset-sale shares are labeled as triangular proposals with proportional closure when their joint sum crosses one, rather than as untouched triangular marginals; because they are countermonotonic transforms of one sampled factor, influence analysis ranks them as one financing-mix axis. Combined runs require a draw count evenly divisible by the population-seed count, replay every matched parameter row across every categorical seed, and build one population at a time so memory stays bounded independently of the number of replicates. The population correlation-ratio effect is reported separately from standardized regression-coefficient rankings because the two measures are not numerically comparable. Send `Accept: application/x-ndjson` to the uncertainty endpoint for progress records before the final result. Replaying the same request and ensemble seed produces the same response.
