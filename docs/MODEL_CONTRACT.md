...
Tax design, scheduled benefits, and revenue disposition are separate policy choices. See [Wealth-tax policy packages and fiscal closures](./FISCAL_CLOSURES.md) for the first-class no-UBI packages, cash-transfer labels, budget identity, and closure-isolated A/B workflow. See [Tax-payment financing calibration](./TAX_PAYMENT_FINANCING.md) for the household-specific cash, borrowing, sale, and loan-service model.
